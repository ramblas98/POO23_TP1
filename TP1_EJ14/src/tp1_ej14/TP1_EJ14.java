 ...
 * generar una sopa de letras y determinar si una palabra dada se encuentra en
 * ella. Resuelve con métodos para cada inciso:
 *
 * a) Generar una matriz de caracteres aleatorios, recibiendo como argumentos:
 * el número de filas, el número de columnas y el rango de valores permitidos
 * (rango al que deben pertenecer los caracteres aleatorios que se elijan como
 * valores). Por ejemplo: ▪ M = 8; N = 10 (M: Filas - N: Columnas) ▪ 65 ≤ A[i,
 * j] ≤ 90 (Código ASCCII)
 *
 * b) Obtener (leer) el carácter de una casilla de una sopa de letras dada.
 *
 * c) Establecer (escribir) el carácter de una casilla de una sopa de letras
 * dada.
 *
 * d) Comprobar si unas coordenadas dadas (número de fila y columna)
 * corresponden a una casilla válida de una sopa de letras dada.
 *
 * e) Comprobar si una palabra dada está en una determinada posición
 * (coordenadas de inicio de búsqueda) en una sopa de letras. Sugerencia:
 * Escribir métodos para realizar búsqueda en los sentidos de los puntos
 * cardinales (NORTE, SUR, ESTE, OESTE, NO, NE, SO y SE).
 *
 */
public class TP1_EJ14 {
    ...
    private static boolean buscarEste(String p, int f, int c) {
        char vec[]=p.toCharArray();
        boolean band=true;
        int i=0;
        while(i<p.length() && band==true){
            if(vec[i]!=sopa[f][c+i]){
                band=false;
            }
            i++;
